...
class ModelRunner(ModelRunnerKVCacheMixin):
    ...
    def _maybe_enable_qwen3_drafter_projection_dispatch(self) -> bool:
        ...
        # prepend: TMA -> SM80 down32 -> SM80 qkv32 -> SM80 gate_up32 ->
        # retained tactic -> production. The exact SM80 dispatches handle
        # disjoint shapes; out32 is intentionally not integrated.
        enabled = False
        ...
        if envs.SGLANG_ENABLE_QWEN3_DRAFTER_SM80_DOWN32.get():
            enabled = self._maybe_enable_qwen3_drafter_sm80_down32() or enabled
    ...
    def _maybe_enable_qwen3_drafter_tma(self) -> bool:
        ...
        Unsupported configurations deliberately retain the production linear
        implementation. Once an exact supported configuration selects TMA,
        ...
        def fallback(reason: str) -> bool:
            logger.warning(
                "Qwen3 drafter TMA requested but unsupported (%s); keeping "
                "the production linear path.",
                reason,
            )
            return False
        ...
        enable = getattr(self.model, "enable_qwen3_drafter_tma", None)
        if not callable(enable):
            return fallback(f"model type={type(self.model).__name__}")
    ...
    def _maybe_enable_qwen3_drafter_sm80_gate_up32(self) -> bool:
        """Enable the selected exact-M32 gate-up kernel on A100 draft workers."""
        from sglang.srt.multiplex.pdmux_context import spec_sm_partition_enabled

        if (
            not envs.SGLANG_ENABLE_QWEN3_DRAFTER_SM80_GATE_UP32.get()
        ...
        def fallback(reason: str) -> bool:
            logger.warning(
                "Qwen3 drafter SM80 gate_up32 requested but unsupported "
        ...
        if hint_mode != 2:
            return fallback(
                "SGLANG_SPEC_PDMUX_SM_HINT must be 2 so non-gate_up32 "
                "fallbacks retain the realized SMALL-width selection"
            )
        ...
        enable = getattr(self.model, "enable_qwen3_drafter_sm80_gate_up32", None)
        if not callable(enable):
            return fallback(f"model type={type(self.model).__name__}")
    ...
    def _maybe_enable_qwen3_drafter_sm80_down32(self) -> bool:
        """Enable the selected exact-M32 down kernel on A100 draft workers."""
        from sglang.srt.multiplex.pdmux_context import spec_sm_partition_enabled

        if (
            not envs.SGLANG_ENABLE_QWEN3_DRAFTER_SM80_DOWN32.get()
            or not self.is_draft_worker
        ):
            return False
        ...
        def fallback(reason: str) -> bool:
            logger.warning(
                "Qwen3 drafter SM80 down32 requested but unsupported "
                "(%s); keeping the existing projection path.",
                reason,
            )
            return False
        ...
        if hint_mode != 2:
            return fallback(
                "SGLANG_SPEC_PDMUX_SM_HINT must be 2 so non-down32 fallbacks "
                "retain the realized SMALL-width selection"
            )
        ...
        enable = getattr(self.model, "enable_qwen3_drafter_sm80_down32", None)
        if not callable(enable):
            return fallback(f"model type={type(self.model).__name__}")

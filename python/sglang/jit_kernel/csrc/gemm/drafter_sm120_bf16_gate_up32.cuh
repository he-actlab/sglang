...
namespace sglang::drafter_sm120_bf16_gate_up32_detail {
...
template <int TileN, int Stages>
inline void drafter_sm120_bf16_gate_up32_schedule(
    tvm::ffi::TensorView output,
    tvm::ffi::TensorView activation,
    tvm::ffi::TensorView weight,
    tvm::ffi::TensorView workspace) {
  using namespace host;
  using namespace sglang::drafter_sm120_bf16_gate_up32_detail;
  ...
  RuntimeCheck(
      is_aligned(activation.data_ptr(), kAlignmentBytes), "drafter SM120 gate_up32 activation must be 16-byte aligned");
  RuntimeCheck(is_aligned(weight.data_ptr(), kAlignmentBytes), "drafter SM120 gate_up32 weight must be 16-byte aligned");
  RuntimeCheck(is_aligned(output.data_ptr(), kAlignmentBytes), "drafter SM120 gate_up32 output must be 16-byte aligned");
  ...
  RuntimeCheck(
      ...
      "drafter SM120 gate_up32 workspace requires ",
      required_workspace_bytes,
      " bytes, got ",
      workspace_bytes.unwrap());
  RuntimeCheck(
      required_workspace_bytes == 0 || is_aligned(workspace.data_ptr(), kAlignmentBytes),
      "drafter SM120 gate_up32 workspace must be 16-byte aligned");
...
#define SGLANG_DRAFTER_GATE_UP32_DEFINE(name, tile_n, stages)                                    \
  inline void name(                                                                              \
      ...
      tvm::ffi::TensorView weight,                                                               \
      tvm::ffi::TensorView workspace) {                                                          \
    drafter_sm120_bf16_gate_up32_schedule<tile_n, stages>(output, activation, weight, workspace); \
  }

SGLANG_DRAFTER_GATE_UP32_DEFINE(drafter_sm120_bf16_gate_up32_n64_s3, 64, 3)
SGLANG_DRAFTER_GATE_UP32_DEFINE(drafter_sm120_bf16_gate_up32_n64_s4, 64, 4)
SGLANG_DRAFTER_GATE_UP32_DEFINE(drafter_sm120_bf16_gate_up32_n64_s5, 64, 5)
SGLANG_DRAFTER_GATE_UP32_DEFINE(drafter_sm120_bf16_gate_up32_n64_s6, 64, 6)
SGLANG_DRAFTER_GATE_UP32_DEFINE(drafter_sm120_bf16_gate_up32_n128_s3, 128, 3)
SGLANG_DRAFTER_GATE_UP32_DEFINE(drafter_sm120_bf16_gate_up32_n128_s4, 128, 4)
SGLANG_DRAFTER_GATE_UP32_DEFINE(drafter_sm120_bf16_gate_up32_n128_s5, 128, 5)
SGLANG_DRAFTER_GATE_UP32_DEFINE(drafter_sm120_bf16_gate_up32_n128_s6, 128, 6)

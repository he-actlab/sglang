...
def _arch_env():
    if not torch.cuda.is_available():
        raise RuntimeError("SM80 BF16 gate_up32 JIT compilation requires CUDA.")
    capability = torch.cuda.get_device_capability()
    if capability != (8, 0):
        raise RuntimeError(
            "SM80 BF16 gate_up32 JIT compilation requires compute capability "
...
@cache_once
def _jit_drafter_sm80_bf16_gate_up32_module() -> Module:
    wrappers = [
        (f"drafter_sm80_bf16_gate_up32_{config}",) * 2 for config in CONFIGS
    ]
    with _arch_env():
        return load_jit(
            "drafter_sm80_bf16_gate_up32",
            cuda_files=["gemm/drafter_sm80_bf16_gate_up32.cuh"],
...
def drafter_sm80_bf16_gate_up32(
    config: str,
    output: torch.Tensor,
    ...
    if not torch.cuda.is_available():
        raise RuntimeError("SM80 BF16 gate_up32 requires CUDA.")
    capability = torch.cuda.get_device_capability(activation.device)
    if capability != (8, 0):
        raise RuntimeError(
            "SM80 BF16 gate_up32 requires compute capability "
            f"8.0, got {capability[0]}.{capability[1]}."
        )
    ...
    module = _jit_drafter_sm80_bf16_gate_up32_module()
    getattr(module, f"drafter_sm80_bf16_gate_up32_{config}")(

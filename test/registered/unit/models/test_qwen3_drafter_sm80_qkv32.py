"""Focused integration gates for the A100 Qwen3 drafter qkv32 kernel."""
...
from sglang.srt.models.qwen3 import (
    ...
    _Qwen3DrafterSm80Qkv32Dispatch,
)
...
class Qwen3DrafterSm80Qkv32ShapeTests(CustomTestCase):
    @staticmethod
    ...
    @staticmethod
    def _linear(k: int = 1024, n: int = 4096):
        return SimpleNamespace(weight=SimpleNamespace(shape=(n, k), device="cuda:0"))

    def test_exact_shape_runs_selected_stage_six(self):
        dispatch = object.__new__(_Qwen3DrafterSm80Qkv32Dispatch)
        ...
        allocate.assert_called_once_with(
            (32, 4096), dtype=torch.bfloat16, device="cuda:0"
        )
        dispatch._run.assert_called_once_with(
            "n128_s6", output, activation, linear.weight, dispatch._workspace
        )
    ...
    def test_other_rows_and_projections_fall_through_without_allocation(self):
        ...
        with (
            ...
            self.assertIsNone(dispatch(self._linear(n=6144), self._activation()))

        allocate.assert_not_called()
        dispatch._run.assert_not_called()


class Qwen3DrafterSm80Qkv32ModelTests(CustomTestCase):
    def test_exact_model_prepends_qkv_without_displacing_gate_dispatch(self):
        class FakeDispatch:
            instance = None
        ...
        class FakeLayer:
            def __init__(self):
                ...
                self.self_attn = SimpleNamespace(
                    ...
                    _drafter_projection_dispatch=gate_dispatch,
                )
        ...
        with (
            ...
            patch(
                "sglang.srt.models.qwen3._Qwen3DrafterSm80Qkv32Dispatch", FakeDispatch
            ),
        ):
            self.assertTrue(Qwen3Model.enable_qwen3_drafter_sm80_qkv32(model, 0))
        ...
        for layer in model.layers:
            for module in (layer.self_attn, layer.mlp):
                ...
                self.assertEqual(combined._dispatches, (dispatch, gate_dispatch))


class ModelRunnerQwen3DrafterSm80Qkv32Tests(CustomTestCase):
    @staticmethod
    def _runner(*, is_draft_worker: bool) -> ModelRunner:
        ...
        runner.model = SimpleNamespace(
            enable_qwen3_drafter_sm80_qkv32=Mock(return_value=True)
        )
        return runner

    def test_env_switch_is_default_off(self):
        self.assertIs(envs.SGLANG_ENABLE_QWEN3_DRAFTER_SM80_QKV32.default, False)
    ...
    def test_default_off_and_target_worker_are_untouched(self):
        for is_draft_worker, enabled in ((True, False), (False, True)):
            ...
            with (
                envs.SGLANG_ENABLE_QWEN3_DRAFTER_SM80_QKV32.override(enabled),
                patch.object(torch.cuda, "get_device_capability") as capability,
            ):
                self.assertFalse(runner._maybe_enable_qwen3_drafter_sm80_qkv32())
            capability.assert_not_called()
            runner.model.enable_qwen3_drafter_sm80_qkv32.assert_not_called()

    def test_exact_a100_draft_role_enables_on_small_stream(self):
        runner = self._runner(is_draft_worker=True)
        small_stream = object()
        with (
            envs.SGLANG_ENABLE_QWEN3_DRAFTER_SM80_QKV32.override(True),
            ...
            self.assertTrue(runner._maybe_enable_qwen3_drafter_sm80_qkv32())

        stream_context.assert_called_once_with(small_stream)
        runner.model.enable_qwen3_drafter_sm80_qkv32.assert_called_once_with(0)

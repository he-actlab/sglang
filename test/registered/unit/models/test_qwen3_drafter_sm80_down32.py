"""Focused integration gates for the A100 Qwen3 drafter down32 kernel."""
...
class Qwen3DrafterSm80Down32ShapeTests(CustomTestCase):
    @staticmethod
    def _activation(m: int = 32, k: int = 3072, alignment: int = 256):
    ...
    @staticmethod
    def _linear(k: int = 3072, n: int = 1024):
        return SimpleNamespace(weight=SimpleNamespace(shape=(n, k), device="cuda:0"))

    def test_constructor_allocates_one_stable_512k_workspace(self):
        workspace = object()
        with (
            patch.object(
            ...
            patch(
                "sglang.jit_kernel.drafter_sm80_bf16_out_down32._jit_drafter_sm80_bf16_out_down32_module"
            ) as compile_module,
            patch.object(torch, "empty", return_value=workspace) as allocate,
        ):
            dispatch = _Qwen3DrafterSm80Down32Dispatch(0)

        compile_module.assert_called_once_with()
        allocate.assert_called_once_with(524288, dtype=torch.uint8, device="cuda")
        self.assertIs(dispatch._workspace, workspace)

    def test_exact_shape_runs_selected_splitk4_stage_five(self):
        dispatch = object.__new__(_Qwen3DrafterSm80Down32Dispatch)
        ...
        allocate.assert_called_once_with(
            (32, 1024), dtype=torch.bfloat16, device="cuda:0"
        )
        dispatch._run.assert_called_once_with(
            "splitk4_n128_s5", output, activation, linear.weight, dispatch._workspace
        )
    ...
    def test_other_rows_and_projections_fall_through_without_allocation(self):
        ...
        with (
            ...
            self.assertIsNone(dispatch(self._linear(n=4096), self._activation()))

        allocate.assert_not_called()
        dispatch._run.assert_not_called()


class Qwen3DrafterSm80Down32ModelTests(CustomTestCase):
    def test_exact_model_prepends_down_without_displacing_existing_dispatches(self):
        class FakeDispatch:
            instance = None
        ...
        qkv_dispatch = Mock(name="qkv_dispatch", return_value=None)
        gate_dispatch = Mock(name="gate_dispatch", return_value=None)
        existing_dispatch = _Qwen3ChainedProjectionDispatch(qkv_dispatch, gate_dispatch)
        ...
        class FakeLayer:
            def __init__(self):
                ...
                self.self_attn = SimpleNamespace(
                    ...
                    _drafter_projection_dispatch=existing_dispatch,
                )
        ...
        with (
            ...
            patch(
                "sglang.srt.models.qwen3._Qwen3DrafterSm80Down32Dispatch", FakeDispatch
            ),
        ):
            self.assertTrue(Qwen3Model.enable_qwen3_drafter_sm80_down32(model, 0))
        ...
        for layer in model.layers:
            for module in (layer.self_attn, layer.mlp):
                ...
                self.assertEqual(
                    combined._dispatches,
                    (dispatch, qkv_dispatch, gate_dispatch),
                )


class ModelRunnerQwen3DrafterSm80Down32Tests(CustomTestCase):
    @staticmethod
    def _runner(*, is_draft_worker: bool) -> ModelRunner:
        ...
        runner.model = SimpleNamespace(
            enable_qwen3_drafter_sm80_down32=Mock(return_value=True)
        )
        return runner

    def test_env_switch_is_default_off(self):
        self.assertIs(envs.SGLANG_ENABLE_QWEN3_DRAFTER_SM80_DOWN32.default, False)
    ...
    def test_default_off_and_target_worker_are_untouched(self):
        for is_draft_worker, enabled in ((True, False), (False, True)):
            ...
            with (
                envs.SGLANG_ENABLE_QWEN3_DRAFTER_SM80_DOWN32.override(enabled),
                patch.object(torch.cuda, "get_device_capability") as capability,
            ):
                self.assertFalse(runner._maybe_enable_qwen3_drafter_sm80_down32())
            capability.assert_not_called()
            runner.model.enable_qwen3_drafter_sm80_down32.assert_not_called()

    def test_exact_a100_draft_role_enables_on_small_stream(self):
        runner = self._runner(is_draft_worker=True)
        small_stream = object()
        with (
            envs.SGLANG_ENABLE_QWEN3_DRAFTER_SM80_DOWN32.override(True),
            ...
            self.assertTrue(runner._maybe_enable_qwen3_drafter_sm80_down32())

        stream_context.assert_called_once_with(small_stream)
        runner.model.enable_qwen3_drafter_sm80_down32.assert_called_once_with(0)

    def test_wrong_architecture_width_or_hint_keeps_existing_path(self):
        cases = (((9, 0), (76, 32), 2), ((8, 0), (64, 44), 2), ((8, 0), (76, 32), 1))
